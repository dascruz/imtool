  ...
  TEST_F(ImageTest, ModifyMaxLevelScalesPixelColors) {
    ...
    for (unsigned long y_pos = 0; y_pos < IMAGE_DIMENSIONS.height; ++y_pos) {
      for (unsigned long x_pos = 0; x_pos < IMAGE_DIMENSIONS.width; ++x_pos) {
        ...
        auto expectedRed = static_cast<unsigned short>(originalPixel.red * newMaxColorValue / originalMaxColorValue);
        auto expectedGreen = static_cast<unsigned short>(originalPixel.green * newMaxColorValue / originalMaxColorValue);
        auto expectedBlue = static_cast<unsigned short>(originalPixel.blue * newMaxColorValue / originalMaxColorValue);
